 ...
 * 封装图片框架 Picasso http://square.github.io/picasso/
 */
public class PicassoUtils {
    ...
    //指定尺寸加载图片
    public static void loadImageWithSize(Context context, String imgUrl, int width, int height, ImageView imageView) {
        Picasso.with(context).load(imgUrl).resize(width, height).centerCrop().into(imageView);
    }

    //在Holder中使用图片加载
    public static void loadImageWithHolder(Context context, String imgUrl, int placeholder, ImageView imageView) {
        Picasso.with(context).load(imgUrl).fit().placeholder(placeholder).into(imageView);
    }

    //对图片自定义裁剪
    public static void loadImageWithCrop(Context context, String imgUrl, ImageView imageView) {
